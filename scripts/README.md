...
| `sync-lock.sh` | **Sync semaphore** over a Tree's worktrees: `check`/`acquire`/`release`/`list`. One `.SYNCING` flag in the branch *container*, never per branch (per-branch is slow and leaves stale flags when a session dies) |
