...
- **1.19** (13 September 2026) — **The delivery layer: a memory that is not delivered does not exist.**
  ...
  **Leaf vs Folio** table. And the community relationship gets its **invocable surface**:
  ...
  counts as published**.
  **Second half of the version — what verifying it turned up, and it is a finding in itself: almost
  nothing was missing. It was the fix made in one place and the old contract still alive in
  another.** The `workbench` correction had to be repeated in the **glossary** (the same obsolete
  definition, in all three languages — and a contradiction there travels further than one in the
  prose, because the glossary is what someone reads to learn the model), in **`manual.md`** (the
  navigable front door, which was also missing `on-task-start` from 1.17), in two **skills** (one
  told you to reference images *from* the work surface — a broken link in every other clone), and
  above all in **`init_roots.sh`**, which kept creating `workbench/` **without its ignore rule**: a
  fix in the prose that leaves the script doing the opposite is not a fix. Running that bootstrap
  end-to-end **for the first time** also showed it `touch`ing three **empty files** that its own
  generated index listed as if they existed — against the **lazy** rule the spec has had since 1.15
  (measured: a full scaffold left ~75% of folders empty; *an empty file committed on day one is
  indistinguishable from a document nobody wrote, and it is what makes a memory look full while it
  is empty*). Sweeping references instead of recalling them found two more: `sync-lock.sh` was
  **named by the spec and absent from the repo**, and one `§` cross-reference pointed at a section
  that does not exist. The same discipline applied to the new commands: `roots-upstream.sh` stated
  the rule in its sentence and **not in its exit code** (a caller could not tell *"handed you a URL"*
  from *"published"*), and its scrub matched `/home` and `/Users` but **not the root the workspace
  that wrote it lived under** — *a scrub with a hole exactly where you work is worse than none: it
  answers clean and you believe it*. Glossary 56 → 66 terms; `domain-keeper.template.md` now takes
  its claim as step 0 (it described a *declared owner* that never touched the semaphore); and the
  seed's own `_meta.json` was wrong in nearly every field. Does not change the `.roots/` format.
...
| `roots-report.py` | **The whole forest at once**: every Tree x Branch, ranked by what can still be **lost** — uncommitted `.roots/` memory first, the bench weighted at zero. Prints its weights and the reason per row. Read-only git queries |
| `roots-seed-audit.py` | **Before re-distributing a bumped seed**: classifies every local copy as FIEL / BANNER / DIVERGE(n) / ORPHAN by hashing it *normalized* against every historical canonical, so an update does not delete contributions nobody sent up |
...
| `roots-report.md` · `roots-seed-audit.md` | **The forest pair**: where is work that can still be lost, and which distributed copies of the seed are forks rather than merely stale |
...
3. DURING the work:
   - Before any batch/sweep: DEMONSTRATE it on one case first — scaling something that
     fails multiplies the error and hides the diagnosis
   - A large corpus REFUTES what already works small; it does not tell you whether it works
